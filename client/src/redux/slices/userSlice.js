  ...
  async (id, { rejectWithValue, getState }) => {
    try {
      ...
      const { data } = await axios.put(
        `${API_URL}/users/farmers/${id}/verify`,
        {},
        config
      );
      return data;
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(verifyFarmer.fulfilled, (state, action) => {
        ...
        // Update user state to reflect verification status change
        state.users = state.users.map((user) => {
          if (user._id === action.payload.data.user) {
            // API returns profile, profile.user is the ID
            return {
              ...user,
              profile: {
                ...user.profile,
